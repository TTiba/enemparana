...
let REDACAO = null;          // api/redacao/index.json — hoje só o nível UF
let RANKING = [];            // api/top_escolas_full/UF/{LOCK_UF}.json (já sem particulares)
...
async function alvoDia1() {
  ...
  if (nivel === "UF") {
    ...
    return { ...d.PUB.resumo.alvo, nome: LOCK_UF_NOME, nivelNome: "Estado" };
  }
  if (nivel === "NRE") {
...
async function refresh() {
  ...
  $("#ent-nome").textContent = alvo.nome || LOCK_UF_NOME;
  ...
  if (nivel === "UF" && REDACAO?.[`UF/${LOCK_UF}`]?.PUB) {
    const r = REDACAO[`UF/${LOCK_UF}`].PUB;
    ...
    $("#red-sz-sub").textContent =
      `indisponível para esta seleção — calculado só para ${LOCK_UF_NOME} inteiro ` +
...
(async () => {
  ...
  const [nreAgg, nreToMuns, muns, redacao, ranking] = await Promise.all([
    ...
    j(`api/top_escolas_full/UF/${LOCK_UF}.json`),
  ]);
  NRE_AGG = nreAgg;
  ...
  const opts = `<option value="">Todos (${LOCK_UF} inteiro)</option>` + nres.map((n) => `<option value="${n}">${n}</option>`).join("");
