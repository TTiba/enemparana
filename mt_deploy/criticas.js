...
/* Quando o nível pedido não tem série própria publicada, carregarDados() cai
 * pro estado. Sem marcar isso, a página diria "Analisando: <escola>" em cima
 * dos números do estado — é o caso de TODA escola num deploy sem
 * api/historico/ESC/. A flag abaixo alimenta o aviso em render(). */
let fallbackUF = null;   // null = sem fallback; senão, o nível que faltou
...
async function carregarNRE(nre) {
  ...
  if (!cds.length) { fallbackUF = "NRE"; return await carregarUF(); }
...
function render() {
  ...
  const NIVEL_NOME = { ESC: "desta escola", MUN: "deste município", NRE: "deste NRE" };
  alvoEl.innerHTML = fallbackUF
    ? `Analisando: <b>${LOCK_UF_NOME}</b> — a série por habilidade `
      + `${NIVEL_NOME[fallbackUF] || "deste recorte"} não está publicada, `
      + `então os números abaixo são os do estado, não os de `
      + `<b>${alvoTxt}</b>.`
    : `Analisando: <b>${alvoTxt}</b>`;
  alvoEl.classList.toggle("alvo-fallback", !!fallbackUF);
  alvoEl.hidden = false;
...
async function initSelects() {
  ...
  NRE_TO_MUNS = (await fetchRegional("data/nre_to_muns.json")) || {};
